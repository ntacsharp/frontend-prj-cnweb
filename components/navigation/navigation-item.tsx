"use client"

import Image from "next/image"
import { useParams,useRouter } from "next/navigation"
import { MyTooltip } from "../tool-tip"
import { cn } from "@/lib/utils"

interface NavigationItemProps{
    id : string;
    imageUrl : string;
    name : string;
}


export const NavigationItem = ({id,imageUrl,name}:NavigationItemProps) =>{

    const params = useParams();
    const router = useRouter();
    function handleOnClick(serverId:string){
        router.push(`/servers/${serverId}`)
    }
    return (
        <div>
            <MyTooltip label ={name} side="right" align="center">
                <button className="group flex relative items-center mb-3" onClick={()=>handleOnClick(id)}>
                    <div className={cn("absolute left-0 bg-primary rounded-r-full transition-all w-[4px]",
                        params?.serverId !== id && "group-hover:h-[20px]",
                        params?.serverId === id ? "h-[36px]" : "h-[8px]"
                    )}/>
                    <div className={cn("relative group flex mx-3 h-[48px] w-[48px] rounded-[24px] group-hover:rounded-[16px] transition-all overflow-hidden",
                        params?.serverId === id && "bg-primary/10 text-primary rounded-[16px]"
                    )}>
                        {imageUrl.startsWith("https://") && <Image
                            fill
                            src ={imageUrl}
                            alt= "Channel Image"
                        />}
                    </div>
                </button>
            </MyTooltip>
        </div>
    )
}